...
val levelPattern =
    Regex("\\b([1-9]|1[0-9]|20)\\b")       //allows numbers between 1 and 20 - https://www.regextutorial.org/regex-for-numbers-and-ranges.php
val abilityScorePattern =
    Regex("\\b([1-9]|[1|2][0-9]|30)\\b")       //allows numbers between 1 and 30
val acPattern =
    Regex("\\b([1-9]|[1|2|3|4][0-9]|50)\\b")       //allows numbers between 1 and 50 (maximum achievable AC is 49)
...
fun calculateHP(charLevel: String, charClass: String, conScore: String): Short {
    ...
    when (charClass) {
        ...
        "Wizard" -> maxHP += (6 + calculateMod(conScore).toInt())
            ...
            0   //for if during character creation con score is changed before class is selected
    }
